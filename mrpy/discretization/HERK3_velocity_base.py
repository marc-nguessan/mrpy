...
class HERK3Scheme(HERKScheme):
    """Base scheme for the implementation of 3-stage Half-Explicit Runge-Kutta
    ...
    # equation
        ...
        if self.uniform: #v_x, v_y, etc are scalars, and we just advance them
            ...
            v_x.sc = self.projection_velocity_x(g_1final, g_33,
            self.B_coefs["b3"]).sc.copy()
            v_y.sc = self.projection_velocity_y(g_2final, g_33,
            self.B_coefs["b3"]).sc.copy()
            ...
            velocity_x.sc = self.projection_velocity_x(g_1final, g_33,
            self.B_coefs["b3"]).sc.copy()
            velocity_y.sc = self.projection_velocity_y(g_2final, g_33,
            self.B_coefs["b3"]).sc.copy()
