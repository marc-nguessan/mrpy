...
from mrpy.discretization.DIRK_base import DIRKScheme
import config as cfg


class ESDIRK4Scheme(DIRKScheme):
    """Base scheme for the implementation of Explicit Singly Diagonally Implicit
    Runge-Kutta with 4 stages (3 implicit internal stages) methods for the
    ...
    def __init__(self, dimension=cfg.dimension, tree_velocity_x=None,
        ...
        DIRKScheme.__init__(self, dimension=dimension,
    ...
    #        DIRKScheme.__init__(self, tree_velocity_x=tree_velocity_x, tree_velocity_y=tree_velocity_y, tree_pressure=tree_pressure, tree_vorticity=tree_vorticity)
    #    else:
    #        DIRKScheme.__init__(self, tree_velocity_x=tree_velocity_x, tree_velocity_y=tree_velocity_y, tree_pressure=tree_pressure)
    ...
    def make_operators(self, tree_velocity_x, tree_velocity_y, tree_pressure,
        ...
        self.hlm_x = self.make_hlm_x("2")
        self.hlm_y = self.make_hlm_y("2")

        self.grad_x = self.make_grad_x("2")
        self.grad_y = self.make_grad_y("2")

        temp = {"hlm_x": self.hlm_x, "hlm_y": self.hlm_y, "grad_x": self.grad_x,
                "grad_y": self.grad_y, "div_x": self.velocity_div_x, "div_y":
                self.velocity_div_y}
        self.operators_dict["2"] = temp
        self.operators_dict["3"] = temp
        self.operators_dict["4"] = temp
    ...
    def make_ksps(self):
        ...
        self.ksp_hlm_x = self.make_ksp_hlm_x("2")
        self.ksp_hlm_y = self.make_ksp_hlm_y("2")
        self.ksp_pc = self.make_ksp_pc("2")

        temp = {"ksp_hlm_x": self.ksp_hlm_x, "ksp_hlm_y": self.ksp_hlm_y,
                "ksp_pc": self.ksp_pc}
        self.ksps_dict["2"] = temp
        self.ksps_dict["3"] = temp
        self.ksps_dict["4"] = temp
    ...
    def advance(self, v_x=None, v_y=None, v_z=None, p=None, t_ini=0, nsp=None,
        ...
        if self.uniform: #v_x, v_y, etc are scalars, and we just advance them
            ...
            if self.st_flag_vc:
                if self.low_mach:
                    ...
                    self.compute_source_term(self.st_tree_vc, self.st_func_vc,
                            t_ini + self.C_coefs["c2"]*self.dt)
                    #mesh.listing_of_leaves(self.st_tree_vc)
                    st_rhs_32 = sd.add_scalars(
                            low_mach_cont,
                            sd.Scalar(self.st_tree_vc))
                    self.compute_source_term(self.st_tree_vc, self.st_func_vc,
                            t_ini + self.C_coefs["c3"]*self.dt)
                    #mesh.listing_of_leaves(self.st_tree_vc)
                    st_rhs_33 = sd.add_scalars(
                            low_mach_cont,
                            sd.Scalar(self.st_tree_vc))
                    self.compute_source_term(self.st_tree_vc, self.st_func_vc,
                            t_ini + self.C_coefs["c4"]*self.dt)
                    #mesh.listing_of_leaves(self.st_tree_vc)
                    st_rhs_34 = sd.add_scalars(
                            low_mach_cont,
                            sd.Scalar(self.st_tree_vc))
                else:
                    mesh.listing_of_leaves(self.st_tree_vc)
                ...
                if self.low_mach:
                    st_rhs_32 = low_mach_cont
                    st_rhs_33 = low_mach_cont
                    st_rhs_34 = low_mach_cont
            ...
            rhs_momentum_x = sd.add_scalars(
                sd.mul_num_scalar(1/self.dt, self.velocity_mass.apply(v_x)),
                sd.mul_num_scalar(self.A_coefs["a21"], self.make_rhs_dae_x(g_11,
                    g_31, st_rhs_12, low_mach_visc_x, one_over_density)))
            rhs_momentum_y = sd.add_scalars(
                sd.mul_num_scalar(1/self.dt, self.velocity_mass.apply(v_y)),
                sd.mul_num_scalar(self.A_coefs["a21"], self.make_rhs_dae_y(g_21,
                    g_31, st_rhs_22, low_mach_visc_y, one_over_density)))

            self.uzawa_solver_internal("2", velocity_x=g_12, velocity_y=g_22,
            ...
            rhs_momentum_x = sd.add_scalars(
                sd.mul_num_scalar(1/self.dt, self.velocity_mass.apply(v_x)),
                sd.mul_num_scalar(self.A_coefs["a31"], self.make_rhs_dae_x(g_11,
                    g_31, st_rhs_13, low_mach_visc_x, one_over_density)),
                sd.mul_num_scalar(self.A_coefs["a32"], self.make_rhs_dae_x(g_12,
                    g_32, st_rhs_13, low_mach_visc_x, one_over_density)))
            rhs_momentum_y = sd.add_scalars(
                sd.mul_num_scalar(1/self.dt, self.velocity_mass.apply(v_y)),
                sd.mul_num_scalar(self.A_coefs["a31"], self.make_rhs_dae_y(g_21,
                    g_31, st_rhs_23, low_mach_visc_y, one_over_density)),
                sd.mul_num_scalar(self.A_coefs["a32"], self.make_rhs_dae_y(g_22,
                    g_32, st_rhs_23, low_mach_visc_y, one_over_density)))

            self.uzawa_solver_internal("3", velocity_x=g_13, velocity_y=g_23,
            ...
            rhs_momentum_x = sd.add_scalars(
                sd.mul_num_scalar(1/self.dt, self.velocity_mass.apply(v_x)),
                sd.mul_num_scalar(self.A_coefs["a41"], self.make_rhs_dae_x(g_11,
                    g_31, st_rhs_14, low_mach_visc_x, one_over_density)),
                sd.mul_num_scalar(self.A_coefs["a42"], self.make_rhs_dae_x(g_12,
                    g_32, st_rhs_14, low_mach_visc_x, one_over_density)),
                sd.mul_num_scalar(self.A_coefs["a43"], self.make_rhs_dae_x(g_13,
                    g_33, st_rhs_14, low_mach_visc_x, one_over_density)))
            rhs_momentum_y = sd.add_scalars(
                sd.mul_num_scalar(1/self.dt, self.velocity_mass.apply(v_y)),
                sd.mul_num_scalar(self.A_coefs["a41"], self.make_rhs_dae_y(g_21,
                    g_31, st_rhs_24, low_mach_visc_y, one_over_density)),
                sd.mul_num_scalar(self.A_coefs["a42"], self.make_rhs_dae_y(g_22,
                    g_32, st_rhs_24, low_mach_visc_y, one_over_density)),
                sd.mul_num_scalar(self.A_coefs["a43"], self.make_rhs_dae_y(g_23,
                    g_33, st_rhs_24, low_mach_visc_y, one_over_density)))

            self.uzawa_solver_internal("4", velocity_x=g_14, velocity_y=g_24,
