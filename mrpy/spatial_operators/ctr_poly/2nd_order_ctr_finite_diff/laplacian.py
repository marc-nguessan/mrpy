        ...
        for row in range(number_of_rows):
            ...
            if mesh.bc_compatible_local_indexes(tree, level, i-1, j, k) is not None:
                ...
                # the finest level for the left flux is the node's level
                    matrix_add(tree, matrix, row, -(dy)/(dx*(dx*dy)), level, i, j, k)
                    matrix_add(tree, matrix, row, (dy)/(dx*(dx*dy)), level, i_left, j_left, k_left)

                else:
                    for n in range(2):
                        matrix_add(tree, matrix, row, -(dy/2)/((dx/2)*(dx*dy)), level+1, 2*i, 2*j+n, 2*k)
                        matrix_add(tree, matrix, row, (dy/2)/((dx/2)*(dx*dy)), level+1, 2*i_left+1, 2*j+n, 2*k)
                ...
                matrix_add(tree, matrix, row, -2*(dy)/(dx*(dx*dy)), level, i, j, k)
            ...
            if mesh.bc_compatible_local_indexes(tree, level, i+1, j, k) is not None:
                ...
                # the finest level for the right flux is the node's level
                    matrix_add(tree, matrix, row, -dy/(dx*(dx*dy)), level, i, j, k)
                    matrix_add(tree, matrix, row, dy/(dx*(dx*dy)), level, i_right, j_right, k_right)

                else:
                    for n in range(2):
                        matrix_add(tree, matrix, row, -(dy/2)/((dx/2)*(dx*dy)), level+1, 2*i+1, 2*j+n, 2*k)
                        matrix_add(tree, matrix, row, (dy/2)/((dx/2)*(dx*dy)), level+1, 2*i_right, 2*j+n, 2*k)
            ...
            if mesh.bc_compatible_local_indexes(tree, level, i, j-1, k) is not None:
                ...
                if index_left in tree.tree_nodes and tree.nisleaf[index_left] \
                    or index_left not in tree.tree_nodes:
                # the finest level for the left flux is the node's level
                    matrix_add(tree, matrix, row, -(dx)/(dy*(dx*dy)), level, i, j, k)
                    matrix_add(tree, matrix, row, (dx)/(dy*(dx*dy)), level, i_left, j_left, k_left)

                else:
                    for m in range(2):
                        matrix_add(tree, matrix, row, -(dx/2)/((dy/2)*(dx*dy)), level+1, 2*i+m, 2*j, 2*k)
                        matrix_add(tree, matrix, row, (dx/2)/((dy/2)*(dx*dy)), level+1, 2*i+m, 2*j_left+1, 2*k)

            elif tree.bc["south"][0] == "dirichlet":
                # the finest level for the left flux is the node's level; this node
                # receives a second contribution because of the boundary condition
                matrix_add(tree, matrix, row, -2*(dx)/(dy*(dx*dy)), level, i, j, k)
            ...
            if mesh.bc_compatible_local_indexes(tree, level, i, j+1, k) is not None:
                ...
                # the finest level for the right flux is the node's level
                    matrix_add(tree, matrix, row, -dx/(dy*(dx*dy)), level, i, j, k)
                    matrix_add(tree, matrix, row, dx/(dy*(dx*dy)), level, i_right, j_right, k_right)

                else:
                    for m in range(2):
                        matrix_add(tree, matrix, row, -(dx/2)/((dy/2)*(dx*dy)), level+1, 2*i+m, 2*j+1, 2*k)
                        matrix_add(tree, matrix, row, (dx/2)/((dy/2)*(dx*dy)), level+1, 2*i+m, 2*j_right, 2*k)
        ...
        for row in range(number_of_rows):
            ...
            if mesh.bc_compatible_local_indexes(tree, level, i-1, j, k) is not None:
                ...
                if index_left in tree.tree_nodes and tree.nisleaf[index_left] \
                    or index_left not in tree.tree_nodes:
                # the finest level for the left flux is the node's level
                    matrix_add(tree, matrix, row, -(dy*dz)/(dx*(dx*dy*dz)), level, i, j, k)
                    matrix_add(tree, matrix, row, (dy*dz)/(dx*(dx*dy*dz)), level, i_left, j_left, k_left)

                else:
                    for o in range(2):
                        for n in range(2):
                            matrix_add(tree, matrix, row, -((dy/2)*(dz/2))/((dx/2)*(dx*dy*dz)), level+1, 2*i, 2*j+n, 2*k+o)
                            matrix_add(tree, matrix, row, ((dy/2)*(dz/2))/((dx/2)*(dx*dy*dz)), level+1, 2*i_left+1, 2*j+n, 2*k+o)

            elif tree.bc["west"][0] == "dirichlet":
                # the finest level for the left flux is the node's level; this node
                # receives a second contribution because of the boundary condition
                matrix_add(tree, matrix, row, -2*(dy*dz)/(dx*(dx*dy*dz)), level, i, j, k)
            ...
            if mesh.bc_compatible_local_indexes(tree, level, i+1, j, k) is not None:
                ...
                # the finest level for the right flux is the node's level
                    matrix_add(tree, matrix, row, -dy*dz/(dx*(dx*dy*dz)), level, i, j, k)
                    matrix_add(tree, matrix, row, dy*dz/(dx*(dx*dy*dz)), level, i_right, j_right, k_right)

                else:
                    for o in range(2):
                        for n in range(2):
                            matrix_add(tree, matrix, row, -(dy/2)*(dz/2)/((dx/2)*(dx*dy*dz)), level+1, 2*i+1, 2*j+n, 2*k+o)
                            matrix_add(tree, matrix, row, (dy/2)*(dz/2)/((dx/2)*(dx*dy*dz)), level+1, 2*i_right, 2*j+n, 2*k+o)
            ...
            if mesh.bc_compatible_local_indexes(tree, level, i, j-1, k) is not None:
                ...
                if index_left in tree.tree_nodes and tree.nisleaf[index_left] \
                    or index_left not in tree.tree_nodes:
                # the finest level for the left flux is the node's level
                    matrix_add(tree, matrix, row, -(dx*dz)/(dy*(dx*dy*dz)), level, i, j, k)
                    matrix_add(tree, matrix, row, (dx*dz)/(dy*(dx*dy*dz)), level, i_left, j_left, k_left)

                else:
                    for o in range(2):
                        for m in range(2):
                            matrix_add(tree, matrix, row, -(dx/2)*(dz/2)/((dy/2)*(dx*dy*dz)), level+1, 2*i+m, 2*j, 2*k+o)
                            matrix_add(tree, matrix, row, (dx/2)*(dz/2)/((dy/2)*(dx*dy*dz)), level+1, 2*i+m, 2*j_left+1, 2*k+o)

            elif tree.bc["south"][0] == "dirichlet":
                # the finest level for the left flux is the node's level; this node
                # receives a second contribution because of the boundary condition
                matrix_add(tree, matrix, row, -2*(dx*dz)/(dy*(dx*dy*dz)), level, i, j, k)
            ...
            if mesh.bc_compatible_local_indexes(tree, level, i, j+1, k) is not None:
                ...
                # the finest level for the right flux is the node's level
                    matrix_add(tree, matrix, row, -dx*dz/(dy*(dx*dy*dz)), level, i, j, k)
                    matrix_add(tree, matrix, row, dx*dz/(dy*(dx*dy*dz)), level, i_right, j_right, k_right)
            ...
            if mesh.bc_compatible_local_indexes(tree, level, i, j, k-1) is not None:
                ...
                if index_left in tree.tree_nodes and tree.nisleaf[index_left] \
                    or index_left not in tree.tree_nodes:
                # the finest level for the left flux is the node's level
                    matrix_add(tree, matrix, row, -(dx*dy)/(dz*(dx*dy*dz)), level, i, j, k)
                    matrix_add(tree, matrix, row, (dx*dy)/(dz*(dx*dy*dz)), level, i_left, j_left, k_left)

                else:
                    for n in range(2):
                        for m in range(2):
                            matrix_add(tree, matrix, row, -(dx/2)*(dy/2)/((dz/2)*(dx*dy*dz)), level+1, 2*i+m, 2*j+n, 2*k)
                            matrix_add(tree, matrix, row, (dx/2)*(dy/2)/((dz/2)*(dx*dy*dz)), level+1, 2*i+m, 2*j+n, 2*k_left+1)

            elif tree.bc["back"][0] == "dirichlet":
                # the finest level for the left flux is the node's level; this node
                # receives a second contribution because of the boundary condition
                matrix_add(tree, matrix, row, -2*(dx*dy)/(dz*(dx*dy*dz)), level, i, j, k)
            ...
            if mesh.bc_compatible_local_indexes(tree, level, i, j, k+1) is not None:
                ...
                # the finest level for the right flux is the node's level
                    matrix_add(tree, matrix, row, -(dx*dy)/(dz*(dx*dy*dz)), level, i, j, k)
                    matrix_add(tree, matrix, row, (dx*dy)/(dz*(dx*dy*dz)), level, i_right, j_right, k_right)

                else:
                    for n in range(2):
                        for m in range(2):
                            matrix_add(tree, matrix, row, -(dx/2)*(dy/2)/((dz/2)*(dx*dy*dz)), level+1, 2*i+m, 2*j+n, 2*k+1)
                            matrix_add(tree, matrix, row, (dx/2)*(dy/2)/((dz/2)*(dx*dy*dz)), level+1, 2*i+m, 2*j+n, 2*k_right)
    ...
    if tree.dimension == 2:
        ...
        for row in range(number_of_rows):
            ...
            if i == 0:
                if tree.bc["west"][0] == "dirichlet":
                    ...
                    scalar.setValue(row, 2*west(coords)*dy/(dx*(dx*dy)), True)
            ...
            if j == 0:
                if tree.bc["south"][0] == "dirichlet":
                    ...
                    scalar.setValue(row, 2*south(coords)*dx/(dy*(dx*dy)), True)
            ...
            if i == 2**level-1:
                if tree.bc["east"][0] == "dirichlet":
                    ...
                    scalar.setValue(row, 2*east(coords)*dy/(dx*(dx*dy)), True)
            ...
            if j == 2**level-1:
                if tree.bc["north"][0] == "dirichlet":
                    ...
                    scalar.setValue(row, 2*north(coords)*dx/(dy*(dx*dy)), True)
        ...
        for row in range(number_of_rows):
            ...
            if i == 0:
                if tree.bc["west"][0] == "dirichlet":
                    coords = mesh.boundary_coords(tree, "west", level, 0, j, k)
                    scalar.setValue(row, 2*west(coords)*dy*dz/(dx*(dx*dy*dz)), True)
            ...
            if j == 0:
                if tree.bc["south"][0] == "dirichlet":
                    coords = mesh.boundary_coords(tree, "south", level, i, 0, k)
                    scalar.setValue(row, 2*south(coords)*dx*dz/(dy*(dx*dy*dz)), True)
            ...
            if k == 0:
                if tree.bc["back"][0] == "dirichlet":
                    ...
                    scalar.setValue(row, 2*back(coords)*dx*dy/(dz*(dx*dy*dz)), True)
            ...
            if i == 2**level-1:
                if tree.bc["east"][0] == "dirichlet":
                    coords = mesh.boundary_coords(tree, "east", level, 0, j, k)
                    scalar.setValue(row, 2*east(coords)*dy*dz/(dx*(dx*dy*dz)), True)
            ...
            if j == 2**level-1:
                if tree.bc["north"][0] == "dirichlet":
                    coords = mesh.boundary_coords(tree, "north", level, i, 0, k)
                    scalar.setValue(row, 2*north(coords)*dx*dz/(dy*(dx*dy*dz)), True)
            ...
            if k == 2**level-1:
                if tree.bc["forth"][0] == "dirichlet":
                    ...
                    scalar.setValue(row, 2*forth(coords)*dx*dy/(dz*(dx*dy*dz)), True)
