"""Configuration module that contains all the global parameters needed to run a
simulation. It might be wise to design specific configuration modules for
specific cases: for example the implicit explicit scheme does not need a "phi"
variable to compute the advancement in time of the pressure, contrary to the
prediction projection scheme.
"""

import random
from math import sin, cos, exp, pi, tanh

# unit of length, half initial vorticity thickness
delta_0 = 0.01

# half velocity difference
delta_u = 0.1
...
lambda_a = 14.13 * delta_0

# number of fundamental eddies
m_i = 2
...
mass_module_name = "mrpy.spatial_operators." + "haar.2nd_order_ctr_finite_diff.mass"

#inverse_mass_module_name = "mrpy.spatial_operators." + "ctr_poly.2nd_order_ctr_finite_diff.inverse_mass"
inverse_mass_module_name = "mrpy.spatial_operators." + "haar.2nd_order_ctr_finite_diff.inverse_mass"
...
def u_exact(x, y, t=0.):
    ...
    return U_0 + delta_u * (tanh(y / delta_0)) #+ \
    #        amplitude*exp(-0.5*(10*y/delta_i)**2)*(random.random() - 0.5))

    #if (y <= delta_i/10) and (y >= - delta_i/10):
    #    return U_0 + delta_u * (tanh(y / delta_0) + \
    #            amplitude*exp(-0.5*(10*y/delta_i)**2)*(random.random() - 0.5))
...
def v_exact(x, y, t=0.):
    ...
    # return -exp(-50*(x**2 + y**2))
    #return amplitude*exp(-0.5*(10*y/delta_i)**2)*(random.random() - 0.5)
